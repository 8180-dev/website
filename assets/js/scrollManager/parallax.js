...
class parallax {
  ...
  setList(targets, options) {
    targets.forEach((el, i) => {
      ...
      this.list[i] = {
        ...
        img: target.querySelector('.js-parallax-image'),
  ...
  update(y = 0) {
    ...
    // ------
    //
    // this.scrollOld === 0 && console.log(`reset`)
    // this._scroll !== this.scrollOld &&
    //   console.log(`now ${this._scroll}`, `old ${this.scrollOld}`)
    // console.log(Math.abs(this.scrollAccOffset.acc) * this.config.zoom)
    ...
    this.list.forEach(el => {
      ...
      if (topPosView < 1 && topPosView > 0.1 && !target.isShow) {
        target.isShow = true
        target.dom.classList.add('is-show')
        console.log('😋 add class', target.dom.classList[0])

        return true
      }

      if ((topPosView < 0.05 || topPosView > 1) && target.isShow) {
        target.isShow = false
        target.dom.classList.remove('is-show')
        console.log('🤯 remove class', target.dom.classList[0])
    ...
    // console.log(this.scrollAccOffset)
  }
